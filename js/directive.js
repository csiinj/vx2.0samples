	...
	mod.directive("input", ["$os", "$nativeCall", function($os, $nativeCall){
		return {
			...
			compile: function(element, attr, transclude){
				...
				if($os.ios5 && type==="number"){
					element[0].setAttribute("type", "numberx");
                    element.bind("click", function(e){
                                      element[0].setAttribute("type", "numberx");   
                                 });
				}else if($os.iphone && type==="number"){
                    
//					element[0].setAttribute("type", "numberx");
                    /*element.bind("click", function(e){
                    document.activeElement = arguments[0].target;
                    //禁止默认键盘弹出
                    e.preventDefault();
...
//						$nativeCall.changeKeyboard();
				...
				}else if(($os.android || $os.wphone) && type==="date"){
					var date = element[0];
					...
					angular.element(ediv).attr("v-bind", attr.vModel);
					angular.element(ediv).bind("click", function(e){
//						e.preventDefault();
						var that = this;
						$nativeCall.datePicker(function(value){
							that.innerHTML = value;
							angular.element(that).trigger("input");
						});
					});
					angular.element(ediv).bind("input", function(){
						date.value = this.innerHTML;
						angular.element(date).trigger("input");
					});
